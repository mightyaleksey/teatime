 ...
 * @param  {string}  copy   Копировать сущность (вместо переноса).
 * @return {promise}
 */
module.exports = function (source, target, copy) {
    ...
    return utils.mkdir(parentDir)
        .then(method.bind(null, sourceDir, targetDir))
        ...
        .then(function (files) {
            return vow.all(files.map(function (filePath) {
                ...
                if (destPath !== filePath) {
                    return method(filePath, destPath);
                }
            }));
        });
};
